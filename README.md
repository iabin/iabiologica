...
├── docs/                    # Documentation
├── EjecucionCalifornia.txt  # Sample output log (California)
├── EjecucionDiabetes.txt    # Sample output log (Diabetes)
└── README.md
````

* `src/genetic.py` controls which dataset is used via the `RAW_DATA_FILE` constant. 
* `src/processed_data/` is created automatically per run using a timestamp and the seed. 
...
All main configuration is at the top of `src/genetic.py`:
...
```text
src/processed_data/2025.12.09_21.35.10_seed.42/
    mae_results.csv
    gplearn_model.json
...
* If you change the dataset:
  ...
  * Keep a `target` column name, or update `RESULT_COLUMN` in `src/genetic.py`.
